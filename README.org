...
Pre-built, self-contained binaries for Linux, macOS, and Windows are attached to every [[https://github.com/dcprevere/org-cli/releases][GitHub release]]. No .NET runtime required.
...
Download =org-win-x64.zip= from the [[https://github.com/dcprevere/org-cli/releases][releases page]], extract =org.exe=, and add its directory to =PATH=.
...
** OpenClaw integration (=org-memory=)

The fastest way to put =org= in front of an AI agent is the bundled [[https://github.com/openclaw/openclaw][OpenClaw]] skill, =org-memory=. It turns this CLI into structured, linked, human-readable memory for your agent: a knowledge graph for the agent itself, plus task management against your own org files.
...
- =k: Sarah prefers morning meetings= -- agent stores a fact in its knowledge base
...
The skill is what makes =org today=, short CUSTOM_IDs, and =org batch= actually useful day-to-day -- agenda queries, graph links, and file-less mutations are the things flat-text agent memory cannot do.

See [[file:integrations/openclaw/README.md][integrations/openclaw/README.md]] for installation, configuration (agent vs human directories, env vars), and the full shortcut table.
