...
Resolution is attempted in that order: number, then =:ID:=, then =:CUSTOM_ID:=, then title match.

When an index exists (see =org index= below), most mutating commands also accept a bare CUSTOM_ID without a file argument. The index resolves the ID to its file automatically:
...
=org add= auto-assigns a CUSTOM_ID when an index database exists. To backfill IDs on existing headlines:

#+begin_example
org custom-id assign [-d <dir>] [--dry-run]
#+end_example
