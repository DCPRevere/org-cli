...
When no =--files= are given, the tool recursively finds all =.org= files under =--directory=. If neither =--files= nor =--directory= is given, the tool checks =ORG_CLI_DIRECTORY=, then the =directories= config key, then falls back to the current working directory.
...
| Variable                         | Effect                              |
|----------------------------------+-------------------------------------|
| =ORG_CLI_DIRECTORY=              | Directories to scan, separated by =:= (or =;= on Windows). Supports =~/=. |
...
- An explicit character position, =pos:<offset>= (with a file).
- An org-id (the value of an =:ID:= property on a headline).
- A CUSTOM_ID (the value of a =:CUSTOM_ID:= property).
- An exact headline title string (matched against parsed headlines in the file).

Use =id:<value>= or =custom:<value>= for explicit identity types. Bare selectors try ID, CUSTOM_ID, then exact title; ambiguity is an error. Numeric text is not interpreted as an offset.

Most mutating commands also accept an ID without a file argument. The owned index is created and refreshed automatically:

#+begin_example
org todo k4t DONE
org schedule k4t 2026-03-15
org tag add k4t urgent
#+end_example
...
Lists all headlines across org files. Filters are combinable. Output includes CUSTOM_ID (or position if none), level stars, title, tags, outline path, and filename. JSON output includes a =custom_id= field.
...
Roam is an optional extension over the CLI-owned =<directory>/.org-index.db= (override with =--db=). It shares parsed Org documents with core queries and never writes Emacs org-roam databases. See [[file:architecture.md][architecture and migration]].
...
* Index, CUSTOM_ID, and full-text search
...
Queries create and refresh the index automatically. Mutations refresh affected files. Content and parser-configuration fingerprints detect changes even when timestamps are unchanged; unchanged documents reuse their parsed snapshot.

** CUSTOM_ID

Headlines can be addressed by short CUSTOM_ID strings (e.g. =k4t=) stored in the standard =:CUSTOM_ID:= property. These are base36 (a-z, 0-9), starting at 3 characters and growing as needed to stay unique across the index.

=org add= always assigns a standard UUID =ID=. CUSTOM_ID stamping remains available for legacy workflows:

#+begin_example
org id stamp [-d <dir>] [--dry-run]
#+end_example

This scans all =.org= files, generates a unique CUSTOM_ID for every headline that lacks one, and writes the =:PROPERTIES:= drawers. Use =--dry-run= to preview without modifying files.

Once headlines have CUSTOM_IDs, most mutating commands work without a file argument:
...
org tag add k4t urgent
org schedule k4t 2026-03-15
org deadline k4t 2026-04-01
org note k4t "Some note text"
org clock in k4t
org clock out k4t
org archive k4t
org read k4t
org property set k4t EFFORT 2h
org property remove k4t EFFORT
#+end_example

The CLI resolves the CUSTOM_ID via the index to find the file. It syncs the directory first to ensure freshness.

CUSTOM_IDs appear in all output: =headlines=, =agenda=, =search=, =fts=, =clock= reports, and mutation responses. In text mode they appear as the leading identifier or in parentheses; in JSON mode as the =custom_id= field.
