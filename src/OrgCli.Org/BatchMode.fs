...
let private parseCommands (json: string) : BatchCommand list =
    ...
    [for i in 0 .. commands.GetArrayLength() - 1 ->
            ...
            else Map.empty
        {
            ...
            Identifier =
                if cmd.TryGetProperty("identifier") |> fst then cmd.GetProperty("identifier").GetString()
                else ""
            Args = args
        }]

let private applyCommand (config: OrgConfig) (content: string) (cmd: BatchCommand) (now: DateTime) : Result<string * int64, CliError> =
    let resolve () = Headlines.resolveHeadlinePos content cmd.Identifier
    let getArg key = Map.tryFind key cmd.Args |> Option.defaultValue ""
    ...
    | "tag-add" ->
        resolve ()
        |> Result.map (fun pos ->
            Mutations.addTag content pos (getArg "tag"), pos)

    | "tag-remove" ->
        resolve ()
        |> Result.map (fun pos ->
            Mutations.removeTag content pos (getArg "tag"), pos)
    ...
    | "property-set" ->
        resolve ()
        |> Result.map (fun pos ->
            Mutations.setProperty content pos (getArg "key") (getArg "value"), pos)

    | "property-remove" ->
        resolve ()
        |> Result.map (fun pos ->
            Mutations.removeProperty content pos (getArg "key"), pos)

    | "note" ->
        resolve ()
        |> Result.map (fun pos ->
            Mutations.addNote content pos (getArg "text") now, pos)

    | "clock-in" ->
        resolve ()
        |> Result.map (fun pos ->
            Mutations.clockIn content pos now, pos)

    | "clock-out" ->
        resolve ()
        |> Result.map (fun pos ->
            Mutations.clockOut content pos now, pos)
...
let executeBatch (config: OrgConfig) (json: string) (files: Map<string, string>) (now: DateTime)
    ...
    let results =
        ...
        |> List.map (fun cmd ->
            let content =
                Map.tryFind cmd.File currentFiles
                |> Option.defaultValue ""
