...
let parseFileSection (content: string) : Keyword list * PropertyDrawer option * OrgLink list =
    ...
    let fileSectionEnd =
        if firstHeadlineMatch.Success then firstHeadlineMatch.Index
        else content.Length
    ...
    let keywords =
        ...
        |> Seq.cast<Match>
        |> Seq.map (fun m -> { Key = m.Groups.[1].Value.Trim(); Value = m.Groups.[2].Value.Trim() })
        |> Seq.toList
    ...
    let properties =
        if propsMatch.Success then
            ...
            let props : Property list =
                ...
                |> Seq.cast<Match>
                |> Seq.map (fun m -> { Property.Key = m.Groups.[1].Value.Trim(); Value = m.Groups.[2].Value.Trim() })
                |> Seq.toList
...
let parseHeadlineSectionWith (headlinePattern: Regex) (text: string) (startPos: int) : HeadlineWithContent option =
    ...
    if lines.Length = 0 then None
        ...
        if not headlineMatch.Success then None
        else
            let level = headlineMatch.Groups.[1].Value.Length
            let todoKeyword =
                let g = headlineMatch.Groups.[2]
                if g.Success && not (String.IsNullOrWhiteSpace g.Value) then Some g.Value else None
            let priority =
            ...
            let planning =
                if contentLines.Length > 0 then
                    ...
                    if firstContentLine.StartsWith("SCHEDULED:") ||
                       firstContentLine.StartsWith("DEADLINE:") ||
                       firstContentLine.StartsWith("CLOSED:") then
            ...
            let links =
                ...
                |> List.map (fun l -> { l with Position = l.Position + contentOffset })
...
let computeBlockRanges (content: string) : (int * int) list =
    let beginPattern = Regex(@"^#\+BEGIN_\w+", RegexOptions.Multiline ||| RegexOptions.IgnoreCase)
    let endPattern = Regex(@"^#\+END_\w+", RegexOptions.Multiline ||| RegexOptions.IgnoreCase)
...
let private buildDocumentFromSections (keywords: Keyword list) (fileProperties: PropertyDrawer option) (fileLinks: OrgLink list) (headlineRegex: Regex) (content: string) : OrgDocument =
...
let computeOutlinePath (headlines: Headline list) (targetHeadline: Headline) : string list =
    ...
    let targetIdx = headlines |> List.tryFindIndex (fun h -> h.Position = targetHeadline.Position)
    ...
    | Some idx ->
        ...
        let rec findAncestors currentIdx currentLevel acc =
            if currentIdx < 0 || currentLevel <= 0 then acc
            else
...
let computeOutlinePathAtPosition (headlines: Headline list) (position: int) : string list =
    ...
    let containingHeadline =
        headlines
        |> List.filter (fun h -> int h.Position <= position)
        |> List.tryLast
