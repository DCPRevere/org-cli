...
let private planningPartsRegex = Regex(@"(SCHEDULED|DEADLINE|CLOSED):\s*((?:<[^>]+>|\[[^\]]+\])(?:--(?:<[^>]+>|\[[^\]]+\]))?)")
...
let private replaceKeywordUsing (regex: Regex) (headlineLine: string) (newState: string option) : string =
    ...
    if not m.Success then headlineLine
    else
        let stars = m.Groups.[1].Value
        let priority = if m.Groups.[3].Success then m.Groups.[3].Value + " " else ""
...
let private getStateUsing (regex: Regex) (headlineLine: string) : string option =
    let m = regex.Match(headlineLine)
    if m.Success && m.Groups.[2].Success then Some m.Groups.[2].Value
    else None
...
let private replaceTagsUsing (regex: Regex) (headlineLine: string) (tags: string list) : string =
    ...
    if not m.Success then headlineLine
        ...
        let stars = m.Groups.[1].Value
        let keyword = if m.Groups.[2].Success then m.Groups.[2].Value + " " else ""
        let priority = if m.Groups.[3].Success then m.Groups.[3].Value + " " else ""
        let title = m.Groups.[5].Value
        let tagStr = if List.isEmpty tags then "" else sprintf " :%s:" (String.Join(":", tags))
...
let private replacePriorityUsing (regex: Regex) (headlineLine: string) (priority: char option) : string =
    ...
    if not m.Success then headlineLine
        ...
        let stars = m.Groups.[1].Value
        let keyword = if m.Groups.[2].Success then m.Groups.[2].Value + " " else ""
        let priorityStr = match priority with Some c -> sprintf "[#%c] " c | None -> ""
        let title = m.Groups.[5].Value
        let tags = if m.Groups.[6].Success then m.Groups.[6].Value else ""
...
let buildPlanningLine (parts: Map<string, string>) : string option =
    if Map.isEmpty parts then None
    else
        let items =
            ["SCHEDULED"; "DEADLINE"; "CLOSED"]
            |> List.choose (fun k ->
                Map.tryFind k parts
                |> Option.map (fun v -> sprintf "%s: %s" k v))
...
let modifyPlanning (content: string) (pos: int64) (modify: Map<string, string> -> Map<string, string>) : string =
    ...
    reassemble { section with PlanningLine = newPlanningLine }
...
let removeProperty (drawer: string) (key: string) : string option =
    ...
    let filtered = lines |> Array.filter (fun line ->
        not (Regex.IsMatch(line.TrimStart(), pattern)))
    if filtered.Length <= 2 then None
    else Some (String.Join("\n", filtered))

let ensureDrawer (section: HeadlineSection) : HeadlineSection =
    match section.PropertyDrawer with
    | Some _ -> section
    | None -> { section with PropertyDrawer = Some ":PROPERTIES:\n:END:" }

// --- Logbook operations ---

let formatStateChange (newState: string) (oldState: string) (now: DateTime) : string =
    let ts = Writer.formatTimestamp {
...
let insertEntry (section: HeadlineSection) (entry: string) : HeadlineSection =
    ...
    | Some lb ->
        ...
        { section with LogbookDrawer = Some newDrawer }
    | None ->
...
let private extractStateUsing (regex: Regex) (content: string) (pos: int64) : HeadlineState =
    ...
    let todo = if m.Success && m.Groups.[2].Success then Some m.Groups.[2].Value else None
    let priority = if m.Success && m.Groups.[4].Success then Some m.Groups.[4].Value.[0] else None
    let title = if m.Success then m.Groups.[5].Value else section.HeadlineLine
    let tags =
        if m.Success && m.Groups.[6].Success then
            m.Groups.[6].Value.Trim().Split([|':'|], StringSplitOptions.RemoveEmptyEntries)
            |> Array.toList
    ...
    let id =
        ...
        |> Option.bind (fun pd -> getProperty pd "ID")
    let planningParts =
        section.PlanningLine
        |> Option.map parsePlanningParts
        |> Option.defaultValue Map.empty
