...
let resolveHeadlinePos (content: string) (identifier: string) : Result<int64, CliError> =
    ...
    | false, _ ->
        ...
        match doc.Headlines |> List.tryFind (fun h -> Types.tryGetId h.Properties = Some identifier) with
...
let computeInheritedTags (config: OrgConfig) (doc: OrgDocument) (target: Headline) : string list =
    let filetags =
        if config.TagInheritance then Types.getFileTags doc.Keywords
        else []
    let ancestorTags =
...
let private alwaysInheritedProperties = set ["CATEGORY"; "ARCHIVE"; "COLUMNS"; "LOGGING"]
...
let private shouldInheritProperty (config: OrgConfig) (key: string) : bool =
    ...
    || (config.PropertyInheritance
        && (config.InheritProperties.IsEmpty
            || config.InheritProperties |> List.exists (fun p -> p.ToUpperInvariant() = upperKey)))
            ...
            | None ->
                ...
                | None ->
                    ...
                    |> Option.map (fun kw ->
                        ...
                        if spaceIdx >= 0 then kw.Value.Substring(spaceIdx + 1).Trim()
                        else "")
                    |> Option.bind (fun v -> if System.String.IsNullOrWhiteSpace v then None else Some v)
    | None -> None

let filterByTagWithInheritance (config: OrgConfig) (docs: (string * OrgDocument) list) (tag: string) (matches: HeadlineMatch list) : HeadlineMatch list =
...
let private formatTagList (tags: string list) : string =
    ...
    else sprintf ":%s:" (System.String.Join(":", tags))

let resolveVirtualProperty (config: OrgConfig) (doc: OrgDocument) (h: Headline) (key: string) (file: string option) : string option =
    match key.ToUpperInvariant() with
    | "ITEM" -> Some h.Title
    | "TODO" -> h.TodoKeyword
    | "PRIORITY" ->
        h.Priority |> Option.map (fun (Priority c) -> string c)
    ...
    | "CATEGORY" ->
        ...
        |> Option.orElseWith (fun () ->
            ...
            |> Option.map (fun kw -> kw.Value))
        |> Option.orElseWith (fun () ->
            file |> Option.map (fun f -> System.IO.Path.GetFileNameWithoutExtension(f)))
    | "SCHEDULED" ->
        h.Planning |> Option.bind (fun p -> p.Scheduled) |> Option.map Writer.formatTimestamp
    | "DEADLINE" ->
        h.Planning |> Option.bind (fun p -> p.Deadline) |> Option.map Writer.formatTimestamp
    | "CLOSED" ->
        h.Planning |> Option.bind (fun p -> p.Closed) |> Option.map Writer.formatTimestamp
    | _ ->
        resolveProperty config doc h key
