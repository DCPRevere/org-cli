...
let toJsonString (node: JsonNode) : string = node.ToJsonString(serializerOptions)
...
let error (err: CliError) : string = toJsonString (errorNode err)
...
let commandDefs: CommandDef list =
    [ { Name = "agenda"
        ...
        Args =
          [ { Name = "subcommand"
              Description = "today, week, or todo"
              Required = false } ]
        Flags = [ ("--tag", "Filter by tag"); ("--state", "Filter TODO items by state") ]
        HelpArgs =
          [ "today       Today's scheduled items, deadlines, and overdue (default)"
            "week        Next 7 days"
            "todo        All TODO items"
            "--tag TAG   Filter by tag"
            "--state ST  Filter TODO items by state" ] }
      { Name = "headlines"
        Description = "List and filter headlines"
        Usage = "headlines [--todo STATE] [--tag TAG] [--level N] [--property K=V]"
        Args = []
        Flags =
          [ ("--todo", "Filter by TODO state")
            ("--tag", "Filter by tag")
            ("--level", "Filter by level")
            ("--property", "Filter by property K=V") ]
        HelpArgs =
          [ "--todo STATE      Filter by TODO state"
            "--tag TAG         Filter by tag"
            "--level N         Filter by headline level"
            "--property K=V    Filter by property" ] }
      { Name = "add"
        Description = "Add a new headline"
        Usage =
          "add <file> <title> [--todo STATE] [--priority P] [--tag TAG] [--scheduled DATE] [--deadline DATE] [--under <id-or-title>]"
        Args =
          [ { Name = "file"
        ...
        Flags =
          [ ("--todo", "TODO state")
            ...
            ("--under", "Parent headline id or title") ]
        HelpArgs =
          [ "--todo STATE      Set TODO state"
            "--priority P      Set priority (A-Z)"
            "--tag TAG         Add tag (repeatable)"
            "--scheduled DATE  Set SCHEDULED (yyyy-MM-dd)"
      ...
      { Name = "search"
        ...
        HelpArgs = [] }
      { Name = "links"
        Description = "List and resolve links in a file"
      ...
      { Name = "roam"
        ...
        Args =
          [ { Name = "subcommand"
              Description = "sync, node, backlinks, tag, link, alias, ref"
              Required = true } ]
        Flags = [ ("--db", "Database path") ]
        HelpArgs =
          [ "sync [--force]                    Sync database with files"
      ...
      { Name = "index"
        Description = "Build or update the headline index"
      ...
      { Name = "fts"
        ...
        Args =
          [ { Name = "query"
              Description = "FTS5 query (boolean, phrase, prefix)"
              Required = true } ]
        Flags =
          [ ("--db", "Database path (default: <dir>/.org-index.db)")
            ("--no-sync", "Skip pre-query incremental sync") ]
        HelpArgs =
...
let jchar (c: char option) : JsonNode =
    match c with
    | Some v -> JsonValue.Create(string v) :> JsonNode
    | None -> null
