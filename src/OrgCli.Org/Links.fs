...
let private resolveFilePath (path: string) (currentFile: string) : string =
    ...
    if dir = null || dir = "" then
        ...
        Path.Combine(dir, path) |> fun p -> p.Replace("\\", "/")
...
let resolveLink
    ...
    | "file" ->
        ...
        | None, _ ->
            { (unresolved link) with
                TargetFile = Some targetPath }
        | Some(f, _), None ->
            { (unresolved link) with
                TargetFile = Some f }
    ...
    | "fuzzy" ->
        if link.Path.StartsWith("*") then
            ...
            let inCurrent =
                currentDoc
                |> Option.bind (fun (f, doc) ->
                    findHeadlineByTitle link.Path f doc |> Option.map (fun (t, p) -> (f, t, p)))
...
let resolveLinksInFile (file: string) (docs: (string * OrgDocument) list) : ResolvedLink list =
    ...
    | Some(_, d) ->
        ...
        d.Links |> List.map (fun (link, _) -> resolveLink link file docs abbrevs)
