...
let shiftTimestamp (ts: Timestamp) (now: DateTime) : Timestamp =
    ...
    | Some (rtype, n, unit) ->
        let newDate =
            match rtype with
            | RepeaterType.Standard ->
                addUnits ts.Date n unit
            | RepeaterType.FromToday ->
        ...
        let newRangeEnd =
            ts.RangeEnd |> Option.map (fun endTs -> { endTs with Date = endTs.Date + delta })
        { ts with Date = newDate; RangeEnd = newRangeEnd }
...
let private hasRepeaterInPlanning (planningParts: Map<string, string>) : (string * Timestamp) option =
    ...
    |> List.tryPick (fun key ->
        ...
        |> Option.bind (fun ts ->
            ts.Repeater
            |> Option.bind parseRepeater
            |> Option.map (fun _ -> (key, ts))))
...
let private getEffectiveLogAction (config: OrgConfig) (loggingProp: string option) (oldState: string option) (newState: string option) (isDone: bool) : LogAction =
    ...
    | _ ->
        ...
        let doneAction =
            if isDone && config.LogDone <> LogAction.NoLog then Some config.LogDone
            else None
        enterAction
...
let private keywordsFromConfig (config: OrgConfig) : string list =
    Types.allKeywords config.TodoKeywords
...
let setTodoState (config: OrgConfig) (content: string) (pos: int64) (newState: string option) (now: DateTime) : string =
    ...
    let isDoneTransition = newState |> Option.map (fun s -> Types.isDoneState config.TodoKeywords s) |> Option.defaultValue false
    let doc = Document.parse content
    let loggingProp =
        doc.Headlines
        |> List.tryFind (fun h -> h.Position = pos)
        |> Option.bind (fun h -> Headlines.resolveProperty config doc h "LOGGING")
    let logAction = getEffectiveLogAction config loggingProp oldState newState isDoneTransition
    ...
    if isRepeat then
        let repeatToState =
            ...
            |> Option.defaultWith (fun () ->
                oldState |> Option.defaultValue "TODO")

        let newHeadlineLine = HeadlineEdit.replaceKeywordWith kws section.HeadlineLine (Some repeatToState)

        let newParts =
            existingParts
            |> Map.map (fun key raw ->
                if key = "CLOSED" then raw
                else
        ...
        let newHeadlineLine = HeadlineEdit.replaceKeywordWith kws section.HeadlineLine newState
...
let private formatRescheduleEntry (keyword: string) (oldRaw: string) (now: DateTime) : string =
    ...
    let label = if keyword = "SCHEDULED" then "Rescheduled" else "New deadline"
...
let setScheduled (config: OrgConfig) (content: string) (pos: int64) (ts: Timestamp option) (now: DateTime) : string =
    ...
    let newPlanningLine = HeadlineEdit.buildPlanningLine newParts
    let shouldLog = config.LogReschedule <> LogAction.NoLog && oldScheduled.IsSome && ts.IsSome
    let result =
        if shouldLog then
            let entry = formatRescheduleEntry "SCHEDULED" oldScheduled.Value now
            HeadlineEdit.insertEntry { section with PlanningLine = newPlanningLine } entry
        else
            { section with PlanningLine = newPlanningLine }
...
let setDeadline (config: OrgConfig) (content: string) (pos: int64) (ts: Timestamp option) (now: DateTime) : string =
    ...
    let newPlanningLine = HeadlineEdit.buildPlanningLine newParts
    let shouldLog = config.LogRedeadline <> LogAction.NoLog && oldDeadline.IsSome && ts.IsSome
    let result =
        if shouldLog then
...
let private addRefileLogEntry (config: OrgConfig) (content: string) (pos: int64) (now: DateTime) : string =
    if config.LogRefile = LogAction.NoLog then content
    else
        let section = HeadlineEdit.split content pos
        let ts = HeadlineEdit.formatInactiveTimestamp now
...
let refile (config: OrgConfig) (srcContent: string) (srcPos: int64) (tgtContent: string) (tgtPos: int64) (sameFile: bool) (now: DateTime) : string * string =
...
let setProperty (content: string) (pos: int64) (key: string) (value: string) : string =
    ...
    let newDrawer = s.PropertyDrawer |> Option.map (fun pd -> HeadlineEdit.setProperty pd key value)
...
let removeProperty (content: string) (pos: int64) (key: string) : string =
    ...
    | Some pd ->
        ...
        HeadlineEdit.reassemble { section with PropertyDrawer = newDrawer }
...
let addTag (content: string) (pos: int64) (tag: string) : string =
    ...
    if List.contains tag existing then content
    else
        let newTags = existing @ [tag]
        let newHeadline = HeadlineEdit.replaceTagsWith kws section.HeadlineLine newTags
        HeadlineEdit.reassemble { section with HeadlineLine = newHeadline }
    ...
    if List.contains tag existing then content
        ...
        let exclusionPeers =
            ...
            |> List.tryPick (fun g ->
                ...
                | _ -> None)
            |> Option.defaultValue []
        let filtered = existing |> List.filter (fun t -> not (List.contains t exclusionPeers))
...
let removeTag (content: string) (pos: int64) (tag: string) : string =
    ...
    let section = HeadlineEdit.split content pos
    let existing = HeadlineEdit.parseTagsWith kws section.HeadlineLine
    if not (List.contains tag existing) then content
    else
...
let clockOut (content: string) (pos: int64) (now: DateTime) : string =
    ...
    | Some lb ->
        ...
        if not found then content
        else
            let newDrawer = String.Join("\n", newLines)
            HeadlineEdit.reassemble { section with LogbookDrawer = Some newDrawer }
...
let formatNewHeadline (title: string) (level: int) (todoState: string option) (priority: char option) (tags: string list) (scheduled: Timestamp option) (deadline: Timestamp option) : string =
...
let addHeadline (content: string) (title: string) (level: int) (todoState: string option) (priority: char option) (tags: string list) (scheduled: Timestamp option) (deadline: Timestamp option) : string =
    let headline = formatNewHeadline title level todoState priority tags scheduled deadline
    let separator = if content.Length > 0 && not (content.EndsWith("\n")) then "\n" else ""
    content + separator + headline

let addHeadlineUnder (content: string) (parentPos: int64) (title: string) (todoState: string option) (priority: char option) (tags: string list) (scheduled: Timestamp option) (deadline: Timestamp option) : string =
...
let private addPropertiesToSubtree (subtreeContent: string) (newProps: (string * string) list) : string =
    ...
    let afterHeadline = 1
    let afterPlanning =
        if afterHeadline < lines.Length && HeadlineEdit.planningLineRegex.IsMatch(lines.[afterHeadline].TrimStart())
        then afterHeadline + 1
        else afterHeadline
    let newPropLines = newProps |> List.map (fun (k, v) -> sprintf ":%s: %s" k v) |> Array.ofList
    let hasDrawer =
...
let archive (srcContent: string) (srcPos: int64) (archiveContent: string) (srcFile: string) (outlinePath: string list) (now: DateTime) : string * string =
    let subtree = Subtree.extractSubtree srcContent srcPos
    let headlineLine = subtree.Split([|'\n'|]).[0]
    let m = headlineRegex.Match(headlineLine)
    let todoState = if m.Success && m.Groups.[2].Success then Some m.Groups.[2].Value else None
    let subtreeHeadlineMatch = Regex.Match(subtree, @"^(\*+) ")
    let currentLevel = if subtreeHeadlineMatch.Success then subtreeHeadlineMatch.Groups.[1].Value.Length else 1
    ...
    let props =
        ...
        ] @ (match todoState with Some s -> [("ARCHIVE_TODO", s)] | None -> [])
    let stampedSubtree = addPropertiesToSubtree adjustedSubtree props
    let separator = if archiveContent.Length > 0 && not (archiveContent.EndsWith("\n")) then "\n" else ""
