...
// Timestamp parsing
let pTimestampType =
    (pchar '<' >>% TimestampType.Active) <|>
    (pchar '[' >>% TimestampType.Inactive)
...
let pDate =
    pipe3
        ...
        (pint32 .>> pchar '-')
        pint32
        (fun y m d -> DateTime(y, m, d))

let pDayName =
    choice [
        pstring "Mon"; pstring "Tue"; pstring "Wed"; pstring "Thu"
        pstring "Fri"; pstring "Sat"; pstring "Sun"
    ]
...
let pTimestampRange : Parser<Timestamp, unit> =
    ...
    |>> fun (start, endOpt) ->
        match endOpt with
        | None -> start
        | Some endTs -> { start with RangeEnd = Some { endTs with RangeEnd = None } }
...
// Headline parsing
let pStars : Parser<int, unit> =
    many1Chars (pchar '*') |>> String.length
...
let pPriority : Parser<Priority, unit> =
    pstring "[#" >>. anyOf "ABCDEFGHIJKLMNOPQRSTUVWXYZ" .>> pchar ']'
    |>> Priority
...
let pPlanningItem =
    pPlanningKeyword .>> ws .>>. pTimestampRange
...
let pPlanningLine : Parser<Planning, unit> =
    parse {
        ...
        let scheduled = items |> List.tryFind (fun (k, _) -> k = "SCHEDULED") |> Option.map snd
        let deadline = items |> List.tryFind (fun (k, _) -> k = "DEADLINE") |> Option.map snd
