...
type OrgRoamDb(dbPath: string) =
    ...
    member _.Initialize() =
        ...
        if isNewDb then
            ...
            let version = versionCmd.ExecuteScalar() :?> int64 |> int

            if version > Domain.DbVersion then
                failwith "Database was created with a newer version of org-roam"
            elif version < Domain.DbVersion then
                failwithf "Database has older schema version %d (expected %d). Delete the database and re-sync." version Domain.DbVersion
    ...
    member _.ClearFile(filePath: string) =
        executeNonQuery "DELETE FROM files WHERE file = @file" ["@file", box filePath] |> ignore
    ...
    member _.InsertAlias(alias: DbAlias) =
        ...
        executeNonQuery sql ["@nodeId", box alias.NodeId; "@alias", box alias.Alias] |> ignore

    member _.GetAliases(nodeId: string) : string list =
        executeReader "SELECT alias FROM aliases WHERE node_id = @nodeId"
            ["@nodeId", box nodeId]
            (fun r -> r.GetString(0))
    ...
    member _.GetTags(nodeId: string) : string list =
        executeReader "SELECT tag FROM tags WHERE node_id = @nodeId"
            ["@nodeId", box nodeId]
            (fun r -> r.GetString(0))
    ...
    member _.InsertRef(ref: DbRef) =
        ...
        executeNonQuery sql [
            "@nodeId", box ref.NodeId
            "@ref", box ref.Ref
            "@type", box ref.Type
        ] |> ignore

    member _.GetRefs(nodeId: string) : (string * string) list =
        executeReader "SELECT ref, type FROM refs WHERE node_id = @nodeId"
            ["@nodeId", box nodeId]
    ...
    member _.GetLinksFrom(nodeId: string) : DbLink list =
        let sql = "SELECT pos, source, dest, type, properties FROM links WHERE source = @source"
