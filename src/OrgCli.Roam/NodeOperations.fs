...
let createFileNode (roamDirectory: string) (options: CreateNodeOptions) : string =
    ...
    let content = Writer.createFileNode nodeId options.Title options.Tags options.Aliases options.Refs
...
let addLink (db: Database.OrgRoamDb) (sourceFilePath: string) (sourceNodeId: string) (targetNodeId: string) (description: string option) : unit =
    ...
    if Types.tryGetId doc.FileProperties = Some sourceNodeId then
        ...
        match doc.Headlines |> List.tryFind (fun h -> Types.tryGetId h.Properties = Some sourceNodeId) with
        | None -> failwithf "Source node %s not found in file %s" sourceNodeId sourceFilePath
        | Some h ->
            ...
            let prefix = if insertPos > 0 && content.[insertPos - 1] <> '\n' then "\n" else ""
...
let addAlias (filePath: string) (nodeId: string) (alias: string) : Result<unit, string> =
    ...
    | Some nodePosition ->
        let newContent = Writer.addToMultiValueProperty content nodePosition "ROAM_ALIASES" alias
...
let removeAlias (filePath: string) (nodeId: string) (alias: string) : Result<unit, string> =
    ...
    | None -> Error (sprintf "Node %s not found in file %s" nodeId filePath)
    | Some nodePosition ->
        let newContent = Writer.removeFromMultiValueProperty content nodePosition "ROAM_ALIASES" alias
        File.WriteAllText(filePath, newContent)
        Ok ()

/// Add a tag to a node
let addTag (filePath: string) (nodeId: string) (tag: string) : unit =
    let content = File.ReadAllText(filePath)
    let doc = Document.parseFile filePath
    ...
    if Types.tryGetId doc.FileProperties = Some nodeId then
        ...
        if not (List.contains tag existingTags) then
            ...
            let hasFiletags = lines |> Array.exists (fun l -> l.TrimStart().ToLower().StartsWith("#+filetags:"))
            ...
            let newContent =
                if hasFiletags then
                    ...
                    // Add after title
                    let titleIdx = lines |> Array.tryFindIndex (fun l -> l.TrimStart().ToLower().StartsWith("#+title:"))
                    match titleIdx with
                    | Some idx ->
        ...
        match doc.Headlines |> List.tryFind (fun h -> Types.tryGetId h.Properties = Some nodeId) with
        | None -> failwithf "Node %s not found in file %s" nodeId filePath
        | Some h ->
            let existingTags = HeadlineEdit.parseTags (content.Substring(int h.Position).Split([|'\n'|]).[0])
            if not (List.contains tag existingTags) then
                ...
                let newContent = HeadlineEdit.reassemble { section with HeadlineLine = newHeadlineLine }
                File.WriteAllText(filePath, newContent)
...
let removeRef (filePath: string) (nodeId: string) (ref: string) : Result<unit, string> =
    ...
    | None -> Error (sprintf "Node %s not found in file %s" nodeId filePath)
    | Some nodePosition ->
        let newContent = Writer.removeFromMultiValueProperty content nodePosition "ROAM_REFS" ref
        File.WriteAllText(filePath, newContent)
        Ok ()

/// Delete a node (either remove headline or delete file)
let deleteNode (db: Database.OrgRoamDb) (nodeId: string) : unit =
    match db.GetNode(nodeId) with
    | None -> failwithf "Node %s not found" nodeId
    | Some node ->
