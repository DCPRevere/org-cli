...
let parseRef (refStr: string) : (string * string) option =
    if String.IsNullOrWhiteSpace(refStr) then None
...
let updateFile (db: Database.OrgRoamDb) (roamDirectory: string) (filePath: string) =
    ...
    if existingHash = Some contentHash then
        ...
        if isFileNode then
            let nodeId = fileNodeId.Value
            let title = fileTitle |> Option.defaultValue (Path.GetFileNameWithoutExtension(fullPath))
            ...
            for refStr in refs do
                ...
                | Some (refType, refPath) ->
                    db.InsertRef({ NodeId = nodeId; Ref = refPath; Type = refType })
                | None -> ()
        ...
        for headline in doc.Headlines do
            ...
            if headlineId.IsSome && not isExcluded then
                let nodeId = headlineId.Value
        ...
        for (link, sourceNodeId) in doc.Links do
            ...
            | Some srcId ->
                ...
                let linkProps =
                    ...
                    else [(":outline", Domain.formatElispList outlinePath)]
...
let getAllNodes (db: Database.OrgRoamDb) : RoamNode list =
    db.GetAllNodes()
    |> List.choose (fun n -> db.PopulateNode(n.Id))

/// Get a node by ID
let getNode (db: Database.OrgRoamDb) (nodeId: string) : RoamNode option =
    db.PopulateNode(nodeId)
