...
let private parseDoc (content: string) = Document.parse content
...
let ``CANCELLED items excluded from overdue`` () =
    let content =
        sprintf "* CANCELLED Cancelled task\nDEADLINE: <%s %s>\n" (yesterday.ToString("yyyy-MM-dd")) (dayName yesterday)
...
let ``SCHEDULED range produces one item per day`` () =
    ...
    let content =
        sprintf
            "* TODO Multi-day task\nSCHEDULED: <%s %s>--<%s %s>\n"
            (feb5.ToString("yyyy-MM-dd"))
            (dayName feb5)
            (feb7.ToString("yyyy-MM-dd"))
            (dayName feb7)
...
let ``Range filtered by date range returns correct subset`` () =
    ...
    let content =
        sprintf
            ...
            (feb5.ToString("yyyy-MM-dd"))
            (dayName feb5)
            (feb10.ToString("yyyy-MM-dd"))
            (dayName feb10)

    let items = datedItemsFromContent content

    let filtered =
        Agenda.filterByDateRange (DateTime(2026, 2, 7)) (DateTime(2026, 2, 9)) items
