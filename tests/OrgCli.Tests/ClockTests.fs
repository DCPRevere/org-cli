        ...
        [ { Start =
          ...
          { Start =
            ...
            End =
              Some
                  { Type = TimestampType.Inactive
                    Date = DateTime(2026, 2, 5, 12, 30, 0)
                    HasTime = true
                    Repeater = None
                    Delay = None
                    RangeEnd = None }
...
let ``collectClockEntriesFromDocs across multiple files`` () =
    ...
    let results =
        Clock.collectClockEntriesFromDocs [ ("a.org", doc1, content1); ("b.org", doc2, content2) ]
