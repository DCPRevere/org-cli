...
let skipReason =
    "Emacs with org-roam not available. Set RUN_EMACS_TESTS=1 and ensure emacs with org-roam is installed."
...
let createTempDir () =
    let path =
        Path.Combine(Path.GetTempPath(), "org-roam-emacs-test-" + Guid.NewGuid().ToString())
...
let runOrgRoamElisp (roamDir: string) (dbPath: string) (elisp: string) (log: string -> unit) =
    let setupElisp =
        sprintf
            "(progn (require 'org-roam) (setq org-roam-directory \"%s\") (setq org-roam-db-location \"%s\") (org-roam-db-sync) %s)"
            (roamDir.Replace("\\", "/"))
            (dbPath.Replace("\\", "/"))
            elisp

    let tempElispFile =
        Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString() + ".el")
    ...
    try
        ...
        if File.Exists(tempElispFile) then
            File.Delete(tempElispFile)
...
type EmacsInteropTests(output: ITestOutputHelper) =
    ...
    member _.``CLI-created database can be read by Emacs org-roam``() =
        ...
        try
            ...
            | None -> Assert.Fail("Failed to query org-roam from Emacs")
        finally
    ...
    member _.``Emacs org-roam can find node created by CLI``() =
        ...
        try
            ...
            let query =
                sprintf "(car (org-roam-db-query [:select title :from nodes :where (= id \"%s\")]))" nodeId
            ...
            | None -> Assert.Fail("Failed to find node in Emacs org-roam")
        finally
            cleanupTempDir tempDir
    ...
    member _.``CLI can read database created by Emacs org-roam``() =
        ...
        try
            ...
            | _ -> Assert.Fail("Emacs org-roam sync failed")
        finally
            cleanupTempDir tempDir
    ...
    member _.``Links created by CLI are visible in Emacs org-roam``() =
        ...
        try
            ...
            let query =
                sprintf "(length (org-roam-db-query [:select * :from links :where (= source \"%s\")]))" sourceId
            ...
            | None -> Assert.Fail("Failed to query links from Emacs")
        finally
            cleanupTempDir tempDir
    ...
    member _.``Aliases created by CLI are visible in Emacs org-roam``() =
        ...
        try
            ...
            let query =
                sprintf "(length (org-roam-db-query [:select * :from aliases :where (= node-id \"%s\")]))" nodeId
            ...
            | None -> Assert.Fail("Failed to query aliases from Emacs")
        finally
            cleanupTempDir tempDir
    ...
    member _.``Tags created by CLI are visible in Emacs org-roam``() =
        ...
        try
            ...
            let query =
                sprintf "(length (org-roam-db-query [:select * :from tags :where (= node-id \"%s\")]))" nodeId
            ...
            | None -> Assert.Fail("Failed to query tags from Emacs")
        finally
            cleanupTempDir tempDir
    ...
    member _.``Round-trip: CLI create, Emacs modify, CLI read``() =
        ...
        try
            ...
            | _ -> Assert.Fail("Emacs re-sync failed")
        finally
            cleanupTempDir tempDir
