...
let private mkDoc content =
    Document.parse content

let private docs = [
    ("notes/main.org", mkDoc "* Main Heading\n:PROPERTIES:\n:ID: id-main-123\n:CUSTOM_ID: custom-main\n:END:\nSome body with [[id:id-other-456][link to other]]\n** Sub Heading\n:PROPERTIES:\n:ID: id-sub-789\n:END:\nBody\n")
    ("notes/other.org", mkDoc "* Other Heading\n:PROPERTIES:\n:ID: id-other-456\n:END:\nBody\n* Second Heading\nNo properties\n")
...
let ``resolve id link finds headline with matching ID`` () =
    let link = { LinkType = "id"; Path = "id-other-456"; Description = None; SearchOption = None; Position = 0 }
...
let ``resolve id link returns None for unknown ID`` () =
    let link = { LinkType = "id"; Path = "nonexistent"; Description = None; SearchOption = None; Position = 0 }
    let result = Links.resolveLink link "notes/main.org" docs Map.empty
...
let ``resolve file link resolves relative path`` () =
    let link = { LinkType = "file"; Path = "other.org"; Description = None; SearchOption = None; Position = 0 }
    let result = Links.resolveLink link "notes/main.org" docs Map.empty
    Assert.Equal(Some "notes/other.org", result.TargetFile)

[<Fact>]
let ``resolve file link with heading search option`` () =
    let link = { LinkType = "file"; Path = "other.org"; Description = None; SearchOption = Some "*Second Heading"; Position = 0 }
    let result = Links.resolveLink link "notes/main.org" docs Map.empty
    Assert.Equal(Some "notes/other.org", result.TargetFile)
...
let ``resolve file link with custom-id search option`` () =
    let link = { LinkType = "file"; Path = "main.org"; Description = None; SearchOption = Some "#custom-main"; Position = 0 }
    let result = Links.resolveLink link "notes/main.org" docs Map.empty
...
let ``resolve fuzzy heading link finds heading in current file`` () =
    let link = { LinkType = "fuzzy"; Path = "*Sub Heading"; Description = None; SearchOption = None; Position = 0 }
    let result = Links.resolveLink link "notes/main.org" docs Map.empty
    Assert.Equal(Some "notes/main.org", result.TargetFile)
...
let ``resolve https link returns without resolution`` () =
    let link = { LinkType = "https"; Path = "//example.com"; Description = None; SearchOption = None; Position = 0 }
    let result = Links.resolveLink link "notes/main.org" docs Map.empty
    Assert.Equal(None, result.TargetFile)
    Assert.Equal(None, result.TargetHeadline)
...
let ``resolve http link returns without resolution`` () =
    let link = { LinkType = "http"; Path = "//example.com"; Description = None; SearchOption = None; Position = 0 }
    let result = Links.resolveLink link "notes/main.org" docs Map.empty
    Assert.Equal(None, result.TargetFile)
...
let ``unresolvable link returns None for target fields`` () =
    let link = { LinkType = "magnet"; Path = "something"; Description = None; SearchOption = None; Position = 0 }
    let result = Links.resolveLink link "notes/main.org" docs Map.empty
    Assert.Equal(None, result.TargetFile)
    Assert.Equal(None, result.TargetHeadline)
...
let ``link abbreviation expands %s in URL`` () =
    ...
    let link = { LinkType = "bug"; Path = "42"; Description = None; SearchOption = None; Position = 0 }
...
let ``link abbreviation without %s appends path`` () =
    ...
    let link = { LinkType = "wiki"; Path = "OrgMode"; Description = None; SearchOption = None; Position = 0 }
    let result = Links.resolveLink link "notes/main.org" docs abbrevs
    Assert.Equal(Some "http://wiki.example.com/OrgMode", result.TargetFile)

[<Fact>]
let ``resolveLinksInFile uses document link abbreviations`` () =
    let content = "#+LINK: bug http://bugs.example.com/%s\n* Heading\nSee [[bug:99][bug 99]]\n"
