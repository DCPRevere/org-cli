...
type PerformanceTests(output: ITestOutputHelper) =
    ...
    let createTempDir () =
        let path =
            Path.Combine(Path.GetTempPath(), "org-roam-perf-" + Guid.NewGuid().ToString())
    ...
    let generateOrgFile (id: string) (title: string) (linkTargets: string list) =
        let links =
            linkTargets
            |> List.map (fun t -> sprintf "Link to [[id:%s][target]]." t)
            |> String.concat "\n"
    ...
    member _.``Sync 100 files completes in reasonable time``() =
        ...
        try
            ...
            for i in 1..100 do
                let linkTargets = if i > 1 then [ sprintf "node-%03d" (i - 1) ] else []

                let content =
                    generateOrgFile (sprintf "node-%03d" i) (sprintf "Note %d" i) linkTargets
            ...
            Sync.sync db tempDir false
            |> List.iter (fun (f, e) -> output.WriteLine(sprintf "Error: %s: %s" f e))
    ...
    member _.``Sync 500 files measures scaling``() =
        ...
        try
            ...
            output.WriteLine(
                sprintf
                    "Sync 500 files: %d ms (%.2f ms/file)"
                    sw.ElapsedMilliseconds
                    (float sw.ElapsedMilliseconds / 500.0)
            )
    ...
    member _.``Parse file with 500 headlines``() =
        ...
        try
            ...
            output.WriteLine(
                sprintf
                    "Parse 500 headlines: %d ms (%.2f ms/headline)"
                    sw.ElapsedMilliseconds
                    (float sw.ElapsedMilliseconds / 500.0)
            )
    ...
    member _.``Query performance with 500 nodes``() =
        ...
        try
            for i in 1..500 do
                let content =
                    generateOrgFile (sprintf "qnode-%04d" i) (sprintf "Query Note %d" i) []
            ...
            output.WriteLine(
                sprintf
                    "100 node lookups: %d ms (%.3f ms/lookup)"
                    sw.ElapsedMilliseconds
                    (float sw.ElapsedMilliseconds / 100.0)
            )
    ...
    member _.``Link-heavy file performance``() =
        ...
        try
            ...
            let links =
                [ 1..200 ]
                |> List.map (fun i -> sprintf "[[id:target-%02d][Link %d]]" ((i % 50) + 1) i)
                |> String.concat " "
    ...
    member _.``Incremental sync is faster than full sync``() =
        ...
        try
            ...
            Assert.True(
                incrementalTime <= initialTime + 100L,
                sprintf
                    "Incremental sync (%d ms) should not be much slower than initial (%d ms)"
                    incrementalTime
                    initialTime
            )
        finally
    ...
    member _.``Backlink query performance``() =
        ...
        try
            ...
            for i in 1..100 do
                let content =
                    generateOrgFile (sprintf "spoke-%03d" i) (sprintf "Spoke %d" i) [ "center" ]
