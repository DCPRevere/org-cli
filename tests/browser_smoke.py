...
from browser_controls import view, select_filter, fill_filter, action, evidence
...
with tempfile.TemporaryDirectory(prefix='org-board-test-') as workspace, tempfile.TemporaryFile(mode='w+') as log:
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            expect(page.locator('#detail .detail-header > .coordination')).to_have_text('ready')
            expect(page.locator('#detail .detail-title')).to_contain_text('<script>')
            page.get_by_role('button', name='Claim task', exact=True).click()
            expect(page.locator('#detail .detail-header > .coordination')).to_have_text('working')
            evidence(page, 'Regression suite passed; behavior inspected.')
            page.get_by_role('button', name='Submit work', exact=True).click()
            expect(page.locator('#detail .detail-header > .coordination')).to_have_text('review')
            ...
            evidence(page, 'Please address the edited requirements.')
            ...
            page.get_by_role('button', name='Claim task', exact=True).click()
            expect(page.locator('#detail .detail-header > .coordination')).to_have_text('working')
            evidence(page, 'Revised requirements tested.')
            page.get_by_role('button', name='Submit work', exact=True).click()
            expect(page.locator('#detail .detail-header > .coordination')).to_have_text('review')
            page.locator('#actor').fill('reviewer')
            evidence(page, 'Acceptance criteria verified independently.')
            page.get_by_role('button', name='Approve', exact=True).click()
            expect(page.locator('#detail .detail-header > .coordination')).to_have_text('done')
            select_filter(page, '#status', 'done')
            ...
            select_filter(page, '#status', 'open')
            expect(page.locator('#list .task')).to_have_count(1)
            page.locator('#list .task').click()
            evidence(page, 'Unsaved human evidence')
            ...
            evidence(page, 'Preserve across views')
            view(page, 'agenda')
            ...
            expect(page.locator('details.agenda-group')).to_have_attribute('open', '')
            page.get_by_text('Unscheduled · 105', exact=True).click()
            fill_filter(page, '#owner-filter', 'daniel')
            ...
            expect(page.locator('#detail .detail-header > .coordination')).to_have_text('cancelled')
            expect(page.locator('body')).to_have_attribute('aria-busy', 'false')
            ...
            select_filter(page, '#status', 'cancelled')
            ...
            with tempfile.TemporaryDirectory(prefix='org-board-local-') as local_workspace:
                ...
                try:
                    ...
                    select_filter(local_page, '#status', 'done')
                    for _ in range(100):
                        if held: break
